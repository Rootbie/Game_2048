...
class Grid2048(Frame):
    def __init__(self):
        Frame.__init__(self)
        self.text = Label(text="Score", bg=BG_CELL_EMPTY, fg=FG_SCORE, font=FONT2, justify=CENTER, width=34, height=1)
        self.text.grid(row=0, sticky=N)
        self.textScore = Label(text="0", bg=BG_CELL_EMPTY, fg=FG_SCORE, font=FONT2, justify=CENTER, width=34, height=1)
        self.textScore.grid(row=1, sticky=N)

        self.grid()  # Creation of the position manager, grid cutting.
        self.master.title("2048 Reverse")
        self.master.bind("<Key>", self.game)
        self.commands = {KEY_UP: down, KEY_DOWN: up, KEY_LEFT: right, KEY_RIGHT: left,
                         "'Up'": down, "'Down'": up, "'Left'": right, "'Right'": left}
        self.score = 0
        self.bestScore = 0
        self.cell_grid = []  # List initializes to store grid cells(Label)
        ...
        for i in range(LEN_GRID):
            for j in range(LEN_GRID):
                ...
                if value_cell == 0:
                    ...
                    self.cell_grid[i][j].configure(text=str(value_cell), bg=DICT_BG_CELL[value_cell], fg=DICT_FG_CELL[value_cell])
...
root.iconbitmap("./Games/V4/2048_logo.ico")
root.rowconfigure(0)
root.rowconfigure(1)
